function [pnt,latGridInDegrees, longGridInDegrees] = GridSphereYH(numPoints,symm)
...
% symm = true for left right symmetry (on y axis)
if ~exist('sy,,','var')
    symm=false;
end
PHI = GoldenRatio();
...
pnt=[triangles(:, 1, 1), triangles(:, 2, 1), triangles(:, 3, 1)];
...
for i = 1:length(pnt)
    pntSt{i}=num2str(pnt(i,:));
end
pntSt=unique(pntSt);

[latGridInDegrees, longGridInDegrees] = ...
    RemoveEqualPairs(latGridInDegrees, longGridInDegrees);
for i=1:length(pntSt);
    PNT(i,1:3)=str2num(pntSt{i});
end
if length(PNT)~=length(latGridInDegrees)
    error('pnt duplicates?')
end
pnt=PNT;
